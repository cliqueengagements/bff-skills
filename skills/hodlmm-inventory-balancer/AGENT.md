...
- **Post-conditions deny by default.** The Bitflow swap uses `PostConditionMode.Deny` with explicit sender STX conditions AND an FT condition for `minimum-output`. The redeploy inherits `hodlmm-move-liquidity`'s contract-level slippage (`min-dlp` ≥ 95%, `max-liquidity-fee` ≤ 5%) because DLP mint/burn in the same tx cannot be expressed as sender-side post-conditions.
...
- **Never surface secrets.** Wallet password is accepted via `--password` or `WALLET_PASSWORD` env var only. It is never logged, echoed, or included in JSON output.
